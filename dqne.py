...
SelfDQNE = TypeVar("SelfDQNE", bound="DQNE")


class DQNE(OffPolicyAlgorithm):
    """
    Deep Q-Network (DQN)
    ...
    def _on_step(self) -> None:
        ...
        # error mod
        buf_len = len(self.ep_info_buffer)
        if buf_len > 1:
            last = self.ep_info_buffer[buf_len - 1]['r']
            prev_last = self.ep_info_buffer[buf_len - 2]['r']
            if last / prev_last > 2.0:
                self.exploration_rate = min(0.95, self.exploration_rate * 3)
